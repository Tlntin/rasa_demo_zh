...
class MatchEntityExtractor(EntityExtractor):
    ...
    def process(self, message: Message, **kwargs):
        """绝对匹配提取实体词"""
        # print("process", message, type(message))
        entities = []
        for entity, value in self.data.items():
            for i in value:
                # print(i, message.data)
                start = message.data["text"].find(i)
                if start != -1:
                    entities.append({
                        "start": start,
                        "end": start + len(i),
        ...
        if self.take_long or self.take_short:
            for i in list(combinations(entities, 2)):
                ...
                if v0 in v1 or v1 in v0:
                    ...
                    if bool(self.take_long) and short in entities:
                        entities.remove(short)
                    if bool(self.take_short) and long in entities:
